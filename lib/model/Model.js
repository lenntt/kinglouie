...
class Model {
    ...
    addLabel(label) {
        ...
        var existing = this.getLabel(label);
        if (!existing) {
    ...
    addState(state) {
        ...
        var existing = this.getState(state);
        if (!existing) {
    ...
    // Depth-first search
    findPath(targetState) {
        targetState = this.getState(targetState);

        var visited = [];
        var stack = [{state: this.initialState, labels: []}];

        function transitionToFringeFn(path) {
            return function(transition) {
                return {
                    state: transition.to,
                    labels: path.concat([transition.label])
                };
            };
        }
